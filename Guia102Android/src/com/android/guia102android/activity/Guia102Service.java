...
public class Guia102Service extends Guia102Activity{
  ...
  public static List<Atividade> getAtividades(Context context, String service)
	  ...
	  File xml = new File(Environment.getExternalStorageDirectory()+"/atividades.xml");
  ...
  public static List<Bairro> getBairros(Context context, String service)
      ...
      if(!AndroidUtils.isNetworkAvailable(context)){
	  System.out.println("VAI LER DO XML");
	  File xml = new File(Environment.getExternalStorageDirectory()+"/bairros.xml");
  ...
  public static List<Empresa> getEmpresas(Context context, String service, String atividadeId, String cidadeId )
	  throws IOException {
      XStream x = new XStream(new DomDriver(ENCODING));
      String url = URL.replace("{tipo}", service);
      ...
      if(!AndroidUtils.isNetworkAvailable(context)){
	  System.out.println("VAI LER DO XML");
	  File xml = new File(Environment.getExternalStorageDirectory()+"/empresas.xml");
  ...
  public Address pegaEndereco(){
      ...
      Location mylocation = man.getLastKnownLocation(LocationManager.GPS_PROVIDER);
      return null;
  }
  
  public static boolean baixaArquivosXml(Context context){
      try{
	  String caminho = Environment.getExternalStorageDirectory().getPath();
	  new FtpService().baixaArquivos("../xmlAndroid",caminho, FTPClient.ASCII_FILE_TYPE, FTPClient.ASCII_FILE_TYPE);
	  return true;
      }catch (Exception e) {
	  return false;
    }
