...
public class RssService extends Service {
	...
	private List<ReleaseParcel> lastRelParcels = new ArrayList<ReleaseParcel>();
	...
	private void checkAndCreateXmlFile() {
		for (int console = 0; console < 4; console++) {
		...
		if (lastReleases != null && lastReleases.size() > 0) {
			...
			if (mustNotify) {
				...
				Notification notification = buildNotification("Today Releases",
						R.drawable.app_icon_scene_rel, lastReleaseTitle,
						lastReleaseTitle, "" + ++notificationsNumber,
						++notificationsNumber);
				nManager.notify(NOTIFICATION_ID, notification);
			} else {
				Log.i(ServiceUtils.LOG_TAG, "There's no new releases");
			}
		}
	}
...
//				 if (true) {
				...
				lastRelParcels.add(builParcelables(release));
				lastReleaseTitle = release.getTitle();
			}
		}
		if(lastRelParcels.size() > 0){
			mustNotify = true;
		}else{
			mustNotify = false;
		}
	...
	private Notification buildNotification(String title, int iconId,
		...
		i.putExtra("relParcels", (ArrayList<ReleaseParcel>) lastRelParcels);
		...
		Notification notification = new NotificationCompat.Builder(
				...
				.setTicker(ticket).setNumber(count).getNotification();
		// .setContent(rv)
		return notification;
	}
	...
	private ReleaseParcel builParcelables(SyndEntry release) {
		...
		relParcel.setDescription(release.getDescription().getValue());
		return relParcel;
	}
